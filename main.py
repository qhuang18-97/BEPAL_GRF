...
parser.add_argument('--right_players', type=int, default=1,
                    help="number of right team players")
...
policy_net = CommNetMLP(args, args.obs_size)
...
def run(num_epochs): 
    ...
    for ep in range(num_epochs):
        ...
        if 'action_loss' in stat.keys():
            print('Action loss: {}'.format(stat['action_loss']))
        if 'value_loss' in stat.keys():
            print('Value loss: {}'.format(stat['value_loss']))
        if 'map_loss' in stat.keys():
            print('Map loss: {}'.format(stat['map_loss']))
        if 'value_loss_g' in stat.keys():
            print('G Value loss: {}'.format(stat['value_loss_g']))
        if 'enemy_comm' in stat.keys():
            print('Enemy-Comm: {}'.format(stat['enemy_comm']))

        if stat['success']>= (0.9 + 0.05*change_counter):
            for param_group in trainer.optimizer.param_groups:
                param_group["lr"] = param_group["lr"] * 0.1
                change_counter += 1
        print('Learning Rate: {}'.format(trainer.optimizer.param_groups[0]['lr']))
