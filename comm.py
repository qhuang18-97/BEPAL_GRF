...
class CommNetMLP(nn.Module):
    ...
    def __init__(self, args, num_inputs):
        ...
        self.mapdecode = nn.Sequential(
            nn.Linear(self.args.hid_size, self.args.hid_size),
            nn.LeakyReLU(),
            nn.Linear(self.args.hid_size, (self.nagents+self.args.right_players+2) * 11),  # 11=3+3+3+2 corrdinates (3), directions (3), ownership (3), identification (2)
            nn.LeakyReLU()
        )
        ...
        '''
        ### finised running in 7.28 best succ 0.85
        self.gcn_encoder = gcn_mod.GCN(6+2, 32, 16, 0.8)
        ...
        self.gcn_encoder = gcn_mod.GAT(6 + 2, 32, 16, 0.1, 0.2, 3)
        self.encoder = nn.Sequential(
            nn.Linear(16, args.hid_size),
            nn.ReLU(),
            nn.Linear(args.hid_size, args.hid_size),
        ...
        # self.value_global = nn.Linear(self.args.hid_size, 1)
    ...
    def forward_state_encoder(self, x):
        ...
        if self.args.recurrent:
            ...
            for i in range(len(x)):
                f_out = self.gcn_encoder(x[i], adj[i])
                node_fmap.append(f_out[torch.where(x[i][:,-1]>0)[0][0]])
    ...
    def forward(self, x, info={}):
        ...
        for i in range(self.comm_passes):
            ...
            if self.args.recurrent:
                ...
                inp = inp.view(batch_size * n, self.hid_size)  #### 9.6 linear encoder
        ...
        '''
        #value_global = self.value_global(hidden_state).sum(1).view(value_head.shape[0], 1)
        value_global = self.value_global(hidden_state).view(value_head.shape[0], value_head.shape[0])
